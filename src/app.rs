...
impl App {
    ...
    pub fn open_file(&mut self, file_path: PathBuf) {
        ...
        let res_user_file = UserFile::from_path(file_path.clone());
        if res_user_file.is_err() {
            println!("Could not load file {:?}", file_path);
        }
        let mut user_file = res_user_file.unwrap();
        user_file.strokes_dirty = true;
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        ctx.input(|i| {
            for event in &i.events {
                // println!("Event : {:?}", event);
                self.input_manager
                    .manage_events(&mut self.state, event.clone());
            }
        });
    }
